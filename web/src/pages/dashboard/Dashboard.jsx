...
export default function Dashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData({
        clubs, matches, players, referees,
        activeSessions: adminStats.activeSessions,
        totalUsers: adminStats.totalUsers,
        totalRatings: adminStats.totalRatings,
      });
    } catch (_) {
      setData(null);
    } finally {
  ...
  const analyticsStats = [
    ...
    },
    {
      ...
      accent: '#ec4899',
      subtext: 'Utakmica, igrača, sudaca, atmosfere',
      icon: (
        <svg viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5">
    ...
    <div className="min-h-full p-8">
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        {analyticsStats.map((s, i) => (
          <StatCard key={s.label} {...s} loading={loading} delay={(stats.length + i) * 60} />
        ))}
      </div>
